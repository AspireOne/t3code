...
describe("server state projection", () => {
  it("removes ephemeral values from cached server configuration", () => {
    const config = {
      ...CONFIG,
      environmentThemes: [
        {
    ...
    expect(stripped).not.toHaveProperty("environmentThemes");
    expect(config.providers[0]).toHaveProperty("rateLimits");
    expect(config).toHaveProperty("environmentThemes");
  });

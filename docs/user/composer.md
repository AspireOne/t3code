...
Compaction cannot be queued; wait for pending work to finish before sending it.
...
While the agent is working, use **Queue message** or **Ctrl+Shift+Enter**
(**Cmd+Shift+Enter** on macOS) to save a follow-up without interrupting it.
Queued messages run in order after a successful turn. Remove an entry or move
it back to the composer to edit it; queued attachments and context are retained.
